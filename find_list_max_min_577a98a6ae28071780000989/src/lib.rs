...
#[cfg(test)]
mod tests {
    use super::{_minimum, _maximum};

    const ERR_MSG: &str = "\nYour result (left) did not match the expected output (right)";

    fn dotest(arr: &[i32], expected_min: i32, expected_max: i32) {
        assert_eq!(_minimum(arr), expected_min, "{ERR_MSG} with function minimum and arr = {arr:?}");
        assert_eq!(_maximum(arr), expected_max, "{ERR_MSG} with function maximum and arr = {arr:?}");
    }
